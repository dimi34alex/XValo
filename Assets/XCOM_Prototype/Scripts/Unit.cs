...
public class Unit : NetworkBehaviour
{
    ...
    [SerializeField][SyncVar] private int ownerPlayerId;
    [SyncVar] private int enemyPlayerId;
    ...
    {
        if (TurnSystem.Instance.IsPlayerTurn(ownerPlayerId))
        {
            actionPoints = ACTION_POINTS_DEFAULT;
            OnActionPointsChanged?.Invoke(this, actionPoints);
        }
    ...
    private void Update()
    {
        //if (authority) Debug.Log($"{gameObject.name} (ID: {GetComponent<NetworkIdentity>().netId}) имеет authority у {NetworkClient.localPlayer}");
    ...
    {
        Debug.Log($"[Unit] {gameObject.name} | AP: {actionPoints}");
        if (!CanAct())
        {
            Debug.Log("CanAct() = false!");
            return false;
        }
        if (IsOwnedBy((int)NetworkClient.localPlayer.netId))
        {
            Debug.Log("IsOwnedBy((int)NetworkClient.localPlayer.netId) = false!");
            return false; // Запрещаем управлять чужими юнитами
        }
        if (actionPoints > 0)
        {
            SpendActionPoint();
...
public void SetOwner(int playerId)
    ...
    public bool CanAct()
    {
        return TurnSystem.Instance.IsPlayerTurn(ownerPlayerId);
    }
