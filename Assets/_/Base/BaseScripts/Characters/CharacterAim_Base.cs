...
public class CharacterAim_Base : MonoBehaviour {
    ...
    private void Awake() {
        ...
        unitSkeletonCompositeWalker_BodyHead = new V_UnitSkeleton_Composite_Walker(vObject, unitSkeleton, GameAssetsOld.UnitAnimTypeEnum.dMarine_Walk, GameAssetsOld.UnitAnimTypeEnum.dMarine_Idle, new[] { "Body", "Head" });
        vObject.AddRelatedObject(unitSkeletonCompositeWalker_BodyHead);
            
        unitSkeletonCompositeWalker_Feet = new V_UnitSkeleton_Composite_Walker(vObject, unitSkeleton, GameAssetsOld.UnitAnimTypeEnum.dMarine_Walk, GameAssetsOld.UnitAnimTypeEnum.dMarine_Idle, new[] { "FootL", "FootR" });
    ...
    public void ShootTarget(Vector3 targetPosition, Action onShootComplete) {
        ...
        if (axis == Axis.XZ) {
            unitSkeleton.ReplaceBodyPartSkeletonAnim(GameAssetsOld.UnitAnimTypeEnum.dMarine_Attack.GetUnitAnim(new Vector3(aimDir.x, aimDir.z)), "Body", "Head");
        } else {
